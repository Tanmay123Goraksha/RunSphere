...
import { fetchTerritoriesInArea, Territory } from '@/services/zonesApi';
import {
...
const geoJsonToMapCoords = (boundary: Territory['boundary']): LatLng[][] => {
    if (!boundary || !boundary.coordinates) return [];

    if (boundary.type === 'Polygon') {
        // Polygon: coordinates is number[][][]
        return (boundary.coordinates as number[][][]).map((ring) =>
    ...
    if (boundary.type === 'MultiPolygon') {
        ...
        const result: LatLng[][] = [];
        for (const polygon of boundary.coordinates as number[][][][]) {
...
const getTerritoryColors = (territory: Territory) => {
...
export default function RunTrackerMap() {
    ...
    const [celebrationData, setCelebrationData] = useState({
        ...
        achievements: [] as Array<{ code: string; name: string; xp_reward: number }>,
    });
    ...
    const appendPoint = async (loc: Location.LocationObject) => {
        ...
        const pointRecord: RunPoint = { latitude: lat, longitude: lon, recorded_at: toIsoTime(loc.timestamp) };
    ...
    const stopTracking = async () => {
        ...
        let capturedCount = 0;
        let xpEarned = 0;
        let unlockedAchievements: Array<{ code: string; name: string; xp_reward: number }> = [];
        ...
        try {
            ...
            capturedCount = (finishResponse as any)?.territory?.captured || 0;
            xpEarned = (finishResponse as any)?.points?.total || 0;
            unlockedAchievements = (finishResponse as any)?.progression?.unlockedAchievements || [];
        } catch {
    ...
    const recoverPersistedRoute = async () => {
        ...
        try {
            ...
            if (remote?.points?.length) {
                const points = remote.points.map((point: any) => ({ latitude: Number(point.latitude), longitude: Number(point.longitude) }));
                setRoute(points);
            }
