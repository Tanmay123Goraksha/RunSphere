...
import { createClubApi, fetchClubs, fetchMyClubs, joinClubApi, leaveClubApi } from '@/services/clubApi';
...
export default function ClubsScreen() {
    ...
    const onCreate = async () => {
        ...
        try {
            ...
            await load();
        } catch (error) { Alert.alert('Error', error?.message || 'Failed to create club'); }
        finally { setCreating(false); }
    };

    const onJoinLeave = async (clubId, isMember) => {
        try {
            if (isMember) await leaveClubApi(clubId);
            else await joinClubApi(clubId);
            await load();
        } catch (error) { Alert.alert('Error', error?.message || 'Action failed'); }
    };
